...
fn generate_const<const D: usize>(mut n: NoiseSource<D>) {
    ...
    let _y = 0.009 + land*0.3 + mount*(1.-0.3);
}

fn generate_simplex<const D: usize>(mut n: NoiseSource<D>) {
...
fn const_bench(c: &mut Criterion) {
    ...
    let noise = NoiseSource::new(ranges.clone(), 0, 1);
